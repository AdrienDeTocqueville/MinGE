...
char *stb_include_string(const char *str, size_t source_len, const char *path_to_includes, const char *filename, char error[256])
{
	...
	for (i = 0; i < num; ++i) {
		...
		strcat(temp, "/");
		strcat(temp, inc_list[i].filename);
		inc = stb_include_file(temp, path_to_includes, error);
		if (inc == NULL) {
...
char *stb_include_file(char *filename, const char *path_to_includes, char error[256])
{
	...
	if (text == NULL) {
		strcpy(error, "Shader include not found: ");
		strcat(error, filename);
		return 0;
	}
	result = stb_include_string(text, len, path_to_includes, filename, error);
	free(text);
	return result;
}
