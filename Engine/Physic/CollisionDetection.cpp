...
{
    vec3 axis = b->getTransform()->position - a->getTransform()->position;
...
Manifold* SphereSphere(Collider* _a, Collider* _b)
{
    ...
    vec3 normal = b->getTransform()->position - a->getTransform()->position;
    ...
    if (epsilonEqual(squaredDistance, 0.0f, EPSILON*EPSILON))
    {
        ...
        man->points[0] = a->getTransform()->position + a->radius * man->normal;
        man->points[1] = b->getTransform()->position - b->radius * man->normal;
