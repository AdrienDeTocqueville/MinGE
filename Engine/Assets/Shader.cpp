...
static const std::unordered_map<std::string, RenderPass::Type> pass_type = {
	...
	{"fwd-base",	RenderPass::Forward},
	{"fwd-add",	RenderPass::Additive},
	...
	for (json::iterator it = PASSES.begin(); it != PASSES.end(); ++it)
	{
		...
		for (auto &macro : PASS["macros"])
		{
			if (macro.is_array())
			{
				...
				uint32_t count_align = next_power_of_two(count);
...
void Shader::addBuiltin(std::string name, unsigned type)
{
...
Program *Shader::updateBuiltins(uint32_t variant_idx, RenderPass::Type pass)
{
