...
nk_sfml_device_create(void)
{
    ...
    dev->attrib_col = glGetAttribLocation(dev->prog, "Color");
    {
        ...
        GL::BindVertexBuffer(dev->vbo);
        GL::BindElementBuffer(dev->ebo);
...
nk_sfml_device_destroy(void)
{
    ...
    GL::DeleteBuffer(dev->vbo);
    GL::DeleteBuffer(dev->ebo);
...
nk_sfml_render(enum nk_anti_aliasing AA, int max_vertex_buffer, int max_element_buffer)
{
    ...
    GL::Viewport(ivec4(0, 0, window_width, window_height));
    GL::Enable(GL::Blend);
...
{
    (void)usr;
    const std::string text = sf::Clipboard::getString().toAnsiString();
    if (!text.empty()) nk_textedit_paste(edit, text.c_str(), text.size());
}
static void
