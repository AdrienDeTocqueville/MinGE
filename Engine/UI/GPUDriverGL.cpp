...
#else
#include "UI/glsl/shader_fill_frag.h"
#include "UI/glsl/shader_fill_path_frag.h"
#include "UI/glsl/shader_v2f_c4f_t2f_t2f_d28f_vert.h"
#include "UI/glsl/shader_v2f_c4f_t2f_vert.h"
#endif
...
namespace ultralight {
	...
	GPUDriverGL::GPUDriverGL(float _scale, bool _msaa) : scale(_scale), msaa(_msaa) {
	...
	void GPUDriverGL::CreateRenderBuffer(uint32_t render_buffer_id,
		...
		entry.fbo_id = GL::GenFramebuffer();
		GL::BindFramebuffer(entry.fbo_id);
		CHECK_GL();
		...
		// Create MSAA FBO
		entry.msaa_fbo_id = GL::GenFramebuffer();
		GL::BindFramebuffer(entry.msaa_fbo_id);
		CHECK_GL();
	...
	void GPUDriverGL::DestroyRenderBuffer(uint32_t render_buffer_id) {
		if (render_buffer_id == 0)
			return;
		RenderBufferEntry& entry = render_buffer_map[render_buffer_id];
		GL::DeleteFramebuffer(entry.fbo_id);
		if (msaa)
			GL::DeleteFramebuffer(entry.msaa_fbo_id);
	...
	void GPUDriverGL::CreateGeometry(uint32_t geometry_id,
		...
		geometry.vbo_vertices = GL::GenBuffer();
		GL::BindVertexBuffer(geometry.vbo_vertices);
		...
		geometry.vbo_indices = GL::GenBuffer();
		GL::BindElementBuffer(geometry.vbo_indices);
	...
	void GPUDriverGL::DrawGeometry(uint32_t geometry_id,
		...
		if (state.enable_scissor)
		{
			...
			GL::Scissor(ivec4(
				(GLsizei)(r.left * scale),
				(GLsizei)(r.top * scale),
				(GLsizei)((r.right - r.left) * scale),
				(GLsizei)((r.bottom - r.top) * scale)
			));
		}
		else {
			glDisable(GL_SCISSOR_TEST);
	...
	void GPUDriverGL::DestroyGeometry(uint32_t geometry_id) {
		GeometryEntry& geometry = geometry_map[geometry_id];
		CHECK_GL();
		GL::DeleteBuffer(geometry.vbo_indices);
		GL::DeleteBuffer(geometry.vbo_vertices);
	...
	void GPUDriverGL::SelectProgram(ProgramType type) {
		...
		if (i != programs_.end()) {
			...
			GL::UseProgram(i->second.program_id);
		}
		else {
	...
	void GPUDriverGL::UpdateUniforms(const GPUState& state) {
		...
		float params[4] = { (float)(difftime(time(NULL), start) / 1000.0), state.viewport_width, state.viewport_height, scale };
	...
	{
		GL::Viewport(ivec4(0, 0,
			static_cast<GLsizei>(round(width * scale)),
			static_cast<GLsizei>(round(height * scale))
		));
	}
