...
bool Model::load_obj()
{
    ...
    if (!wavefront)
    {
        Error::add(FILE_NOT_FOUND, "Model::load_obj() -> " + path);
        return false;
    }
    ...
    while (getline(wavefront, line))
    {
        ...
        else if (line == "usemtl")
        {
            ...
            submeshes.push_back( Submesh(GL_TRIANGLES, vertices.size(), 0, Material::get(line)) );
        }
        else if (line == "mtllib")
        {
            stream >> line;
...
bool Model::load_mtl(std::string _path)
{
    ...
    if (!mtl)
    {
        Error::add(FILE_NOT_FOUND, "Model::load_mtl() -> " + _path);
        return false;
    }
